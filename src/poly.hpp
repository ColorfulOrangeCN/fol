...
namespace fol {
  ...
  class Poly : public Container {
   ...
   public:
    ...
    Poly<Container, CovSysT> der() {
      Poly<Container, CovSysT> g, &f = *this;
      g.resize(f.size());
      for (int i = 1; i < f.size(); ++i)
        g[i - 1] = i * f[i] % mods;
      return g;
    }
    Poly<Container, CovSysT> inte() {
      Poly<Container, CovSysT> g, &f = *this;
      g.resize(f.size());
      g[0] = 0;
      for (int i = 1; i < f.size(); ++i)
        g[i] = f[i - 1] * CovSysT::inv(i) % mods;
      return g;
    }
    Poly<Container, CovSysT> ln() {
      return (this->inv() *= this->der()).inte();
    }
    Poly<Container, CovSysT> exp() {
      const Poly& f = *this;
      Poly g, tf, lng;
      g.push_back(1);
      for (int n = 1; n <= f.size(); n <<= 1) {
